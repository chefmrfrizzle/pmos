...
`PrivateEgressReviewCase` preserves the existence and prior outcome of any legacy public-web attempt involving an `imported_private` entity. It references the governed job without copying entity names, URLs, or evidence values. `PrivateEgressReviewEvent` stores append-only maker/checker transitions bound to a redacted evidence-package hash. Unattempted legacy jobs are terminally quarantined; attempted jobs remain historically intact and require security review.
...
`RelationshipMentionResolution` versions each proposed target linkage separately. Its identity-package hash binds the mention, exact evidence, relation rule, and material source/target identity fields. `RelationshipMentionResolutionEvent` preserves proposal, rejection, and approval. Only an independent approval of the unchanged package moves the mention to `TARGET_LINKED` and creates a review-required relationship candidate.

`RelationshipMentionReviewBatch` freezes a universe/status cohort into a manifest hash. Batch items retain the mention state and a fingerprint over current source/target identity fields, evidence, and the active resolution. Named `RelationshipMentionReviewAssignment` records expire or may be revoked; every decision is bound to one frozen item and one assignment through `RelationshipMentionReviewDecisionBinding`. A proposal changes the fingerprint, so checker approval requires a newly frozen `TARGET_PROPOSED` batch.
