...
def test_adjudication_is_two_stage_and_preserves_source_entities():
    ...
    with factory() as db:
        ...
        assign_identity_reviewer(db,batch.id,"maker","RESEARCHER","assigner","Maker assigned for identity proposal");assign_identity_reviewer(db,batch.id,"checker","REVIEWER","assigner","Checker assigned for independent approval")
        with pytest.raises(AdjudicationInputError):adjudicate(db,item.id,"PROPOSE_MATCH","maker","RESEARCHER","same official domain and jurisdiction",expected_version=version,review_batch_id=batch.id)
        result=adjudicate(db,item.id,"PROPOSE_MATCH","maker","RESEARCHER","same official domain and jurisdiction",evidence_ids=[evidence.id],expected_version=version,review_batch_id=batch.id)
        assert result["resulting_state"]=="PROPOSED"
        with pytest.raises(AdjudicationInputError):adjudicate(db,item.id,"APPROVE_MATCH","maker","RESEARCHER","self approval",evidence_ids=[evidence.id],review_batch_id=batch.id)
        adjudicate(db,item.id,"APPROVE_MATCH","checker","REVIEWER","independent review completed",evidence_ids=[evidence.id],expected_version=result["version"],review_batch_id=batch.id)
...
def test_adjudication_rejects_stale_review_version():
    ...
    with factory() as db:
        ...
        with pytest.raises(StaleReviewError):adjudicate(db,item.id,"DEFER","reviewer","RESEARCHER","needs more evidence",expected_version="stale")
...
def test_identity_proposal_rejects_overlapping_active_cluster():
    ...
    with factory() as db:
        from pmos_research.identity_review_batch import freeze_identity_batch
        from pmos_research.identity_review_assignment import assign_identity_reviewer
        item,evidence=_review_fixture(db);decision=db.get(ResolutionDecision,item.resolution_decision_id);batch=freeze_identity_batch(db,"assigner","test");assign_identity_reviewer(db,batch.id,"maker","RESEARCHER","assigner","Maker assigned for identity proposal")
        ...
        with pytest.raises(AdjudicationInputError,match="already belongs"):
            adjudicate(db,item.id,"PROPOSE_MATCH","maker","RESEARCHER","Candidate appears to match but has an existing cluster",evidence_ids=[evidence.id],review_batch_id=batch.id)
        assert item.status=="PENDING"
...
def test_identity_approval_requires_the_exact_reviewed_pair_cluster():
    ...
    with factory() as db:
        ...
        item.status="PROPOSED";digest=hashlib.sha256(evidence.content_hash.encode()).hexdigest();event=AdjudicationEvent(queue_item_id=item.id,action="PROPOSE_MATCH",prior_state="PENDING",resulting_state="PROPOSED",reviewer="maker",rationale="legacy proposal",evidence_hash=digest);db.add(event);db.flush();batch=freeze_identity_batch(db,"assigner","test",status="PROPOSED");assign_identity_reviewer(db,batch.id,"checker","REVIEWER","assigner","Checker assigned for independent approval");batch_item=db.scalar(select(IdentityReviewBatchItem).where(IdentityReviewBatchItem.batch_id==batch.id,IdentityReviewBatchItem.queue_item_id==item.id));db.add(IdentityReviewDecisionBinding(batch_item_id=batch_item.id,adjudication_event_id=event.id));db.flush()
        with pytest.raises(AdjudicationInputError,match="no proposed identity cluster"):
            adjudicate(db,item.id,"APPROVE_MATCH","checker","REVIEWER","Independent review must not approve a cluster sharing only one member",evidence_ids=[evidence.id],review_batch_id=batch.id)
