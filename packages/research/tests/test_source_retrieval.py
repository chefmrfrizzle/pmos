...
from pmos_research.db import Base,Claim,Entity,ResearchDocumentSnapshot,ResearchPassageCandidate,ResearchSourceCandidate,SourceDocument
...
def test_candidate_retrieval_queues_exact_passages_without_creating_claims():
    ...
    with factory() as db:
        ...
        result=persist_retrieved_candidate(db,candidate,{"status":"ok","url":candidate.source_url,"title":"Strategy","text":text,"hash":"wire-hash"});db.commit()
