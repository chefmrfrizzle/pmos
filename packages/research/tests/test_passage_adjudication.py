...
def test_passage_support_requires_exact_value_and_independent_approval():
    ...
    with factory() as db:
        ...
        with pytest.raises(PassageAdjudicationError):adjudicate_passage(db,candidate.id,"PROPOSE_SUPPORT","maker","The passage establishes the legal identity","Invented Holdings LP",candidate.status,review_batch_id=batch.id)
        proposal=adjudicate_passage(db,candidate.id,"PROPOSE_SUPPORT","maker","The passage explicitly states the legal identity","Example Capital LP",candidate.status,review_batch_id=batch.id)
        replacement=freeze_review_batch(db,"assigner","venture_capital",status="SUPPORT_PROPOSED")
        with pytest.raises(PassageAdjudicationError,match="proposal's frozen review batch"):adjudicate_passage(db,candidate.id,"APPROVE_SUPPORT","checker","A different assignment cannot approve the existing proposal","Example Capital LP",proposal["resulting_state"],replacement.id)
        with pytest.raises(PassageAdjudicationError):adjudicate_passage(db,candidate.id,"APPROVE_SUPPORT","maker","Attempting self approval of the passage","Example Capital LP",proposal["resulting_state"],batch.id)
        with pytest.raises(PassageAdjudicationError):adjudicate_passage(db,candidate.id,"APPROVE_SUPPORT","checker","Independent review with altered value","Example Capital",proposal["resulting_state"],batch.id)
        result=adjudicate_passage(db,candidate.id,"APPROVE_SUPPORT","checker","Independent review confirms the exact passage and scope","Example Capital LP",proposal["resulting_state"],batch.id);db.commit()
...
def test_material_contradiction_must_be_recorded_as_conflict():
    ...
    with factory() as db:
        ...
        batch=freeze_review_batch(db,"assigner","venture_capital")
        with pytest.raises(PassageAdjudicationError):adjudicate_passage(db,candidate.id,"PROPOSE_SUPPORT","maker","Official sources present contradictory legal identities","Example Capital LP",candidate.status,review_batch_id=batch.id)
        result=adjudicate_passage(db,candidate.id,"MARK_CONFLICT","maker","Official sources present contradictory legal identities","Example Capital LP",candidate.status,review_batch_id=batch.id);db.commit()
...
def test_passage_support_fails_closed_when_snapshot_or_passage_integrity_breaks():
    engine=create_engine("sqlite://");Base.metadata.create_all(engine);factory=sessionmaker(bind=engine,expire_on_commit=False)
    with factory() as db:
        _,candidate=_fixture(db);passage=db.get(EvidencePassage,candidate.evidence_passage_id);passage.passage_hash="0"*64;db.flush();batch=freeze_review_batch(db,"assigner","venture_capital")
        with pytest.raises(PassageAdjudicationError,match="integrity"):
            adjudicate_passage(db,candidate.id,"PROPOSE_SUPPORT","maker","The passage would otherwise support identity","Example Capital LP",candidate.status,review_batch_id=batch.id)
        result=adjudicate_passage(db,candidate.id,"REJECT","maker","Evidence integrity failed and cannot support this assertion",expected_status=candidate.status,review_batch_id=batch.id)
