...
def _claim(db,entity,field,value,rank="S0",group="registry"):
    text=f"The {group} reviewed record states: {value}.";digest=hashlib.sha256(text.encode()).hexdigest();document=SourceDocument(entity_id=entity.id,publisher=group,publisher_independence_group=group,source_rank=rank,source_type="registry",source_url=f"https://{group}.example/{field}",content_hash=digest);db.add(document);db.flush();passage=EvidencePassage(document_id=document.id,section=field,passage=text,passage_hash=digest);db.add(passage);db.flush();claim=Claim(entity_id=entity.id,field=field,value=value,source_url=document.source_url,source_type=document.source_type,confidence=.9,verification_status="SUPPORTED",extractor="test",evidence_hash=digest);db.add(claim);db.flush();db.add(ClaimEvidence(claim_id=claim.id,passage_id=passage.id,directness=.9,supports=True));db.flush();return claim

def _approve_claim_publisher(db,claim):
    link=db.scalar(select(ClaimEvidence).where(ClaimEvidence.claim_id==claim.id));passage=db.get(EvidencePassage,link.passage_id);document=db.get(SourceDocument,passage.document_id);domain=document.source_url.split("//",1)[1].split("/",1)[0]
    assessment=propose_publisher_independence(db,domain,document.publisher_independence_group,"publisher-maker","Publisher control evidence reviewed for the gate",[passage.id]);adjudicate_publisher_independence(db,assessment.id,"APPROVE","publisher-checker","Independent review confirms publisher control grouping","HUMAN_REVIEW_REQUIRED")
...
def test_private_sale_gates_require_exact_evidence_maker_checker_and_counsel():
    ...
    with factory() as db:
        ...
        one=_claim(db,asset,"restitution_review","No identified restitution claim","S1","specialist-one");two=_claim(db,asset,"restitution_review","No identified restitution claim","S2","specialist-two");submit_gate_evidence(db,gates["restitution"].id,[one.id,two.id],"researcher");assert not gate_sufficiency(db,gates["restitution"].id)["sufficient"] and gate_sufficiency(db,gates["restitution"].id)["unreviewed_publisher_count"]==2;_approve_claim_publisher(db,one);_approve_claim_publisher(db,two);adjudicate_gate(db,gates["restitution"].id,"PROPOSE_PASS","maker","RESEARCHER","Independent restitution evidence is attached","EVIDENCE_COLLECTED")
