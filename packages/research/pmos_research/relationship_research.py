...
def adjudicate_relationship_mention(session,mention_id:int,action:str,actor:str,rationale:str,expected_status:str,target_entity_id:int|None=None):
    ...
    if prior=="ENTITY_RESOLUTION_REQUIRED" and action=="PROPOSE_TARGET":
        ...
        package=mention_identity_package_hash(session,mention,target);resolution=RelationshipMentionResolution(mention_candidate_id=mention.id,target_entity_id=target.id,version=max((x.version for x in versions),default=0)+1,status="HUMAN_REVIEW_REQUIRED",proposed_by=actor,proposal_rationale=rationale.strip(),identity_package_hash=package);session.add(resolution);session.flush();session.add(RelationshipMentionResolutionEvent(resolution_id=resolution.id,action=action,prior_state=None,resulting_state=resolution.status,actor=actor,rationale=rationale.strip(),identity_package_hash=package));result="TARGET_PROPOSED"
        ...
        session.add(RelationshipMentionResolutionEvent(resolution_id=resolution.id,action=action,prior_state="HUMAN_REVIEW_REQUIRED",resulting_state=resolution.status,actor=actor,rationale=rationale.strip(),identity_package_hash=resolution.identity_package_hash))
