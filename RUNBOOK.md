...
The public-web corroboration and deep-source queues categorically exclude `imported_private`. Run `python scripts/run_isolated_job.py private-egress-quarantine` after upgrading a legacy datastore: unattempted private homepage and deep-source jobs become terminally quarantined without network access, while any historical attempted homepage job receives an aggregate-governed security review case. Containment never converts a historical attempt into approval and never deletes its audit history.

Authorized security staff inspect redacted legacy-attempt packets at `GET /security/private-egress-reviews`. The packet exposes hashed domain, GET method, query count, credential flag, path-segment count, attempt count, outcome, and error type—not the entity name or URL. An `ADMIN`/`COUNSEL` with `security:write` proposes escalation or no-material-disclosure treatment; a different principal with `security:approve` must approve the unchanged evidence package. Query-bearing or credential-bearing URLs cannot be closed as no-material disclosure.
...
Every relationship-candidate action requires `review_batch_id` for an active assigned evidence batch containing the candidate’s exact passage. Universe permission alone is insufficient. The candidate event is bound to the batch item and assignment so later assurance can prove who was authorized at decision time.
...
The freezer excludes passage candidates already present in another active frozen batch. Repeating identical criteria with no new candidates returns the existing batch; overlapping active review authority is not created. Close an erroneous or superseded batch before freezing a corrected population.

Before any assignment, copy `config/reviewer-roster.example.json` to encrypted private storage and have the identity owner approve real OIDC subjects, exact permissions, universes, and purposes. Set `PMOS_REVIEWER_ROSTER_PATH`, then run `python scripts/run_isolated_job.py reviewer-roster`. The preflight rejects wildcards, mixed maker/checker authority, self-assigning administrators, stale approval, duplicate subjects, repository-hosted rosters, and missing three-way maker/checker/assigner coverage. Its report contains aggregate coverage only, never subject identities. `READY` does not create assignments or approve evidence.
