...
def test_passage_review_api_enforces_universe_scope_and_maker_checker(monkeypatch):
    ...
    maker=Principal("maker",frozenset({"RESEARCHER"}),frozenset({"evidence:review","evidence:write"}),frozenset({"pension"}),"oidc","maker-correlation");main.app.dependency_overrides[authenticate_private_request]=lambda:maker
    try:
        ...
        checker=Principal("checker",frozenset({"REVIEWER"}),frozenset({"evidence:approve"}),frozenset({"pension"}),"oidc","checker-correlation");main.app.dependency_overrides[authenticate_private_request]=lambda:checker
        with TestClient(main.app) as client:
        ...
        router=Principal("router",frozenset({"RESEARCHER"}),frozenset({"evidence:routing:review","evidence:routing:write"}),frozenset({"pension"}),"oidc","router-correlation");main.app.dependency_overrides[authenticate_private_request]=lambda:router
        with TestClient(main.app) as client:
