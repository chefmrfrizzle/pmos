...
def test_export_request_api_enforces_scope_ownership_and_independent_approval(monkeypatch):
    ...
    requester=Principal("requester",frozenset({"RESEARCHER"}),frozenset({"exports:request","exports:read"}),frozenset({"private_bank"}),"oidc","request-correlation","tenant-a",frozenset({"internal diligence"}),"internal diligence");main.app.dependency_overrides[authenticate_private_request]=lambda:requester
    try:
        ...
        exporter=Principal("exporter",frozenset({"EXPORTER"}),frozenset({"exports:approve","exports:read"}),frozenset({"private_bank"}),"oidc","approval-correlation","tenant-a",frozenset({"internal diligence"}),"internal diligence");main.app.dependency_overrides[authenticate_private_request]=lambda:exporter
        with TestClient(main.app) as client:
