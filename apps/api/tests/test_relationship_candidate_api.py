...
def test_relationship_mention_api_requires_registered_scoped_target(monkeypatch):
    ...
    monkeypatch.setattr(main,"SessionLocal",factory);monkeypatch.setattr(main,"init_db",lambda:None);principal=Principal("resolver",frozenset({"RESEARCHER"}),frozenset({"relationships:review","identity:write"}),frozenset({"venture_capital","private_equity"}),"oidc","correlation","tenant-a",frozenset({"relationship review"}),"relationship review");main.app.dependency_overrides[authenticate_private_request]=lambda:principal
    try:
        with TestClient(main.app) as client:
        ...
        with TestClient(main.app) as client:
            linked=client.post(f"/relationship-mentions/{mention['id']}/actions",json={"action":"APPROVE_TARGET","rationale":"Independent identity review confirms the registered target","expected_status":"TARGET_PROPOSED"});assert linked.status_code==200 and linked.json()["status"]=="TARGET_LINKED" and linked.json()["resulting_candidate_id"]
    finally:main.app.dependency_overrides.clear()
