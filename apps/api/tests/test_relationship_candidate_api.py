...
def test_relationship_candidate_api_is_scoped_and_never_auto_verifies(monkeypatch):
    ...
    with factory() as db:
        source=Entity(name="Alpha Capital",canonical_name="alpha capital",universe="venture_capital");target=Entity(name="Beta Ventures",canonical_name="beta ventures",universe="venture_capital");db.add_all([source,target]);db.flush();text="Alpha Capital entered a strategic partnership with Beta Ventures.";digest=hashlib.sha256(text.encode()).hexdigest();sc=ResearchSourceCandidate(entity_id=source.id,source_url="https://alpha.example/news",source_domain="alpha.example",document_type="RELATIONSHIPS",target_predicates_json='["relationship"]',discovered_from_url="https://alpha.example",discovery_score=90,status="RETRIEVED_REVIEW_REQUIRED");db.add(sc);db.flush();document=SourceDocument(entity_id=source.id,publisher="alpha.example",publisher_independence_group="alpha.example",source_rank="S1",source_type="official_website",source_url=sc.source_url,content_hash=digest);db.add(document);db.flush();db.add(ResearchDocumentSnapshot(source_candidate_id=sc.id,source_document_id=document.id,normalized_text=text,text_hash=digest));passage=EvidencePassage(document_id=document.id,passage=text,passage_hash=digest);db.add(passage);db.flush();db.add(ResearchPassageCandidate(source_candidate_id=sc.id,evidence_passage_id=passage.id,predicate="relationship",confidence=.9));db.flush();discover_relationship_candidates(db);batch=freeze_review_batch(db,"assigner","venture_capital");assign_reviewer(db,batch.id,"maker","RESEARCHER","assigner","Maker assigned relationship candidate review");batch_id=batch.id;db.commit()
    ...
    try:
        with TestClient(main.app) as client:
            ...
            result=client.post(f"/relationship-candidates/{candidate['id']}/actions",json={"action":"PROPOSE_ASSERTION","rationale":"Exact passage supports specialist relationship review","expected_status":"HUMAN_REVIEW_REQUIRED","review_batch_id":batch_id});assert result.status_code==200 and result.json()["status"]=="ASSERTION_PROPOSED" and result.json()["resulting_assertion_id"]
