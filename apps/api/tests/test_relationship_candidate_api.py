...
def test_relationship_mention_api_requires_registered_scoped_target(monkeypatch):
    ...
    monkeypatch.setattr(main,"SessionLocal",factory);monkeypatch.setattr(main,"init_db",lambda:None);admin=Principal("admin",frozenset({"ADMIN"}),frozenset({"identity:review","identity:assign"}),frozenset({"venture_capital","private_equity"}),"oidc","admin-correlation","tenant-a",frozenset({"relationship review"}),"relationship review");main.app.dependency_overrides[authenticate_private_request]=lambda:admin
    try:
        with TestClient(main.app) as client:
